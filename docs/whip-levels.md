...
- **PER-CUT là per-cut (chốt 23/06):** `indexLongFootage` trả **1 cảnh / 1 cut** (KHÔNG gộp ngữ nghĩa ở đây);
  `temporalNodes` = per-cut → mỗi cut 1 thẻ "cảnh này là gì". Trước đây `resegmentTile` gộp ngay tại index →
  thẻ Cut bị trùm nhiều cut (sai tầng). Gộp đã chuyển lên Tầng 1.
...
- **GỘP Ở ĐÂY, derive on-the-fly (chốt 23/06):** `deriveLevelModel` gọi `segmentSemanticUnits` GỘP per-cut node
  (Tầng 0) → pill cha "montage cùng ý → 1", `sub` = số cut con. Gộp là việc VIEW của tầng này, KHÔNG mutate
  Tầng 0 → nhảy tầng = đổi cách VẼ cùng graph (Moat #5), tầng Cut vẫn nguyên per-cut.
...
### ✅ IMPLEMENTATION HIỆN TẠI (20/06, chốt sau nhiều vòng với Phong) — `src/components/LevelView.tsx`

Sau nhiều vòng thử (floor-picker-cạnh-timeline → Z-dolly → giếng-kính), CHỐT mô hình **VIEW riêng 4 tầng onion**:
- **4 tầng:** 3 Cảm-xúc (range heatmap gradient + bezier) · 2 Câu-chuyện (arc Hook/Proof/CTA) · 1 Ý-nghĩa (pill cảnh,
  strip clip-con) · 0 Cut (clip + caption track). Floor-picker iso the-Sims bên trái (neumorphic token).
- **Onion (KO Z-dolly):** tầng active hiện đầy ở trên, tầng kề = ghost mờ **đứng yên** sau (thấy layer khác, ko bay
  cả stack). Z-dolly bị loại vì vi phạm SOTA animated-transition (Heer&Robertson: tránh nhiều motion độc lập + che khuất).
- **Đổi tầng = per-pill morph** qua **View Transitions API** (object constancy) — chỉ pill biến hình clip↔card↔arc,
  nền đứng yên. Tab↑ / Shift+Tab↓.
- **Hierarchy thật:** N clip con (Cut) → pill cha (Ý-nghĩa) → arc (Câu-chuyện) → heatmap (Cảm-xúc). Cùng trục thời gian.
- **Pill thống nhất 1 kiểu** (shell trắng + LED stripe theo vai + strip thumbnail bo góc). Màu vai nhất quán mọi tầng.
- **Data THẬT:** `project.temporalNodes` (Ý-nghĩa) · `tracks/clips`+caption (Cut) · `styleGraphs[active].curves.energy`
  (Cảm-xúc) · `.beats`/archetype (Câu-chuyện). Đọc-only + click pill → seek/select.
- **Spec đầy đủ:** `frontend/whip/design-proto/levels-prototype.html` (prototype vanilla, có cả drag-drop reorder +
  cascade + entity filter + cut-density + emotion heatmap — phần CHƯA port sang LevelView, xem "Còn lại").

**CÒN LẠI (chưa port từ prototype → LevelView, là milestone sau, cần Phong xem footage thật):**
- Drag-drop reorder pill + **cascade apply vào store thật** (mutate temporalNodes/clips) — RỦI RO mutate graph, hoãn
  có chủ đích để làm khi Phong validate footage thật (tránh hỏng project khi unattended). Lõi `engine/cascade.ts` đã có + test.
- Cut nhiều track break-nhỏ (VO đứt đoạn/SFX/Music/Grade) như prototype; entity filter; emotion heatmap đầy đủ; smart-link arc.

### 🔧 (lịch sử) bản giữa — floor-picker cạnh timeline + morph-in-Timeline (đã GỠ, thay bằng VIEW riêng ở trên)

Bản P1 đầu (`LevelView.tsx` — 3 hàng tĩnh trong **tab riêng**) SAI: nhồi 3 lane vào 1 view tách rời timeline thật
→ rối, không truyền được "đang ở tầng nào". **Bản đúng = 2 thứ, KHÔNG view mới:**

1. **Floor-picker kiểu the Sims** (`src/components/FloorPicker.tsx`) — cột hẹp ~120px BÊN HÔNG timeline: 3 "sàn"
   exploded-isometric chồng nghiêng (CSS `rotateX/rotateZ/translateZ`, **không lib 3D**). Sàn đang chọn nổi lên +
   sáng accent + glow; sàn xa lùi sâu + mờ. Click 1 sàn → set `activeLevel`. Đọc-là-hiểu có 3 tầng & đang ở đâu.
2. **Timeline THẬT morph tại chỗ** — `Timeline.tsx` đọc `activeLevel` → đổi cách *nhấn-mạnh* cùng graph qua CSS
   transition ~220ms: Cut(0) = clip thường; Ý-nghĩa(1) = clip mờ-lùi + SEM pill nổi to/rõ; Cảm-giác(2) = clip mờ
   sâu + overlay **đường năng lượng** (`styleGraphs[active].curves.energy` fallback `temporalNodes.energy`) vẽ trùng
   trục thời gian (secToX). KHÔNG cắt cảnh, KHÔNG view mới, KHÔNG re-decode.

Store: `levelMode` (bật/tắt chế-độ-tầng) + `activeLevel: 0|1|2`. Tab **đảo "🧠 Tầng"** = toggle `levelMode` (bỏ
viewTab "levels"/`LevelView` cũ). Phím **Tab** xoay `activeLevel` khi đang ở chế-độ-tầng (FloorPicker + phím cùng
1 nguồn chân lý). Floor-picker + timeline gói chung `.tl-dock` (flex-row) ở đáy.
...
## 7. Lộ trình (phased)
1. ✅ **P1 — Nhảy tầng (xem) = floor-picker the Sims + morph timeline tại chỗ** (REWRITE 20/06, thay bản
   `LevelView.tsx` tab-riêng bị rối). `src/components/FloorPicker.tsx` (sàn exploded-iso CSS, click → activeLevel)
   + `Timeline.tsx` morph 220ms (clip mờ-lùi · SEM pill nổi · đường năng lượng tầng 2) đọc graph có sẵn, ko
   re-decode. Store `levelMode`+`activeLevel`; tab "🧠 Tầng" toggle; phím Tab xoay tầng. `LevelView.tsx` đã xoá.
   tsc/build sạch · ui-sweep 0 err. ⏳ **CÒN hở:** floor-picker chưa hiện nhãn dạng kể chuyện + "vì sao bản dựng"
   (rationale Moat #4 — bản LevelView cũ có, cần port lại vào panel floor-picker khi có graph thật).
2. 🟡 **P2 — Reorder + cascade.** ✅ **LÕI cascade DONE**: `src/engine/cascade.ts` (`cascade`/`colorLanes`/
   ...
   11/11. ⏳ **CÒN: wire drag-drop UI (kéo section ở tầng 1 reorder) + apply vào store thật** (mutate
   temporalNodes/clips/captions — để session tỉnh táo, rủi ro cao khi unattended).
